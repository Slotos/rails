...
module Arel
  module Nodes
    class HomogeneousInTest < Arel::Spec
      def test_in
      ...
      def test_impossible_when_in_empty
        table = Arel::Table.new :users, type_caster: fake_pg_caster

        assert_not_predicate Arel::Nodes::HomogeneousIn.new(["Bobby", "Robert"], table[:name], :in), :impossible?
        assert_predicate Arel::Nodes::HomogeneousIn.new([], table[:name], :in), :impossible?
      end
